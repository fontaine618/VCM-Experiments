...
DIR = "./experiments/grant_sims4ratio/"
...
problems = list(
  synthetic_ar=CJ(
    ...
    random_effect_variance=c(1.), # with and without random effect
...
algorithms = list(
  LSVCMM=data.table(
    ...
    boot=c(F),
    adaptive=c(1.),
    estimate_variance_components=c(F, F, F, F),
    kernel=c("rbf", "rbf", "rbf", "epa"),
    random_effect=c(F, T, F, T),
    kernel_scale=c(0, 0.1, 0.1, 1e-5, 0.05),
    lambda=c(-1, -1, -1, -1),
    name=c("TV-Independent (rbf)", "TV-Longitudinal (proxy, rbf)", "Cross-sectional", "TV-Longitudinal (proxy, epa)")
  )
)
...
#     random_effect=c(F, T, F, T),
#     kernel_scale=c(0.1, 0.1, 1e-5, 0.1),
#     lambda=c(-1, -1, -1, -1),
...
#     boot=c(F),
#     adaptive=c(1.),
#     estimate_variance_components=c(F, F, F, F, F),
#     random_effect=c(F, T, F, T, F),
#     kernel=c("rbf", "rbf", "rbf", "epa", "epa"),
#     kernel_scale=c(0.1, 0.1, 1e-5, 0.15, 0.15),
#     lambda=c(-1, -1, -1, -1, -1),
#     name=c("independent,rbf", "proxy,rbf", "local", "proxy,epa", "independent,epa")
#   )
# )
...
# DIR = "./experiments/grant_sims4b/"
loadRegistry(DIR)

source("./batchtools/problems/synthetic_ar.R")
instance = makeJob(1)$instance
...
# estpars = reduceResultsDataTable(fun = function(result) result$log$kernel_scale) %>% unwrap()
# estpars %<>% left_join(parameters %>% select(job.id, algo_display, prob_display), by="job.id")
# with(estpars, table(result.1, algo_display, prob_display))
