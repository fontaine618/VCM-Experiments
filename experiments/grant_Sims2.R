...
algorithms = list(
  LSVCMM=data.table(
    ...
    boot=c(F),
    adaptive=c(1.),
    estimate_variance_components=c(F, F, F, T, T),
    random_effect=c(F, T, F, T, T),
    kernel_scale=c(0.3, 0.3, 1e-5, 0.3, 0.3),
    lambda=c(-1, -1, -1, -1, 0.),
    name=c("independent", "proxy", "local", "estimated", "dense")
  )
)
...
for(i in seq_along(probs)){
  ...
  gest = ggplot() + 
    ...
    geom_boxplot(
      data=estmat %>% filter(prob_display==prob, algo_display %in% c(
        "LSVCMM (independent)",
        "LSVCMM (local)",
        "LSVCMM (estimated)",
        "LSVCMM (dense)",
        ...
        algo_display=ifelse(algo_display=="LSVCMM (independent)", "TV-Independent", algo_display)
      ) %>% mutate(
        algo_display=ifelse(algo_display=="LSVCMM (local)", "Cross-sectional", algo_display)
      ) %>% mutate(
        algo_display=ifelse(algo_display=="LSVCMM (estimated)", "TV-Longitudinal", algo_display)
      ) %>% mutate(
        algo_display=ifelse(algo_display=="LSVCMM (dense)", "TV-Longitudinal (dense)", algo_display)
      ) %>% mutate(
        algo_display=ifelse(algo_display=="LSVCMM (proxy)", "TV-Longitudinal (proxy)", algo_display)
      ),
      mapping=aes(x=time2, y=estimate, 
                  color=algo_display, 
                  fill=algo_display,
...
glegend = ggplot(
  data=sqerrmat %>% filter(prob_display==probs[1], algo_display %in% c(
    "LSVCMM (independent)",
    "LSVCMM (local)",
    "LSVCMM (estimated)",
    "LSVCMM (dense)",
