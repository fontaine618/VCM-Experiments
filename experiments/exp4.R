...
algorithms = list(
  LSVCMM=data.table(
    selection=c("aic"),
    boot=c(F),
    adaptive=c(1., 1., 1., 1., 0.),
    estimate_variance_components=c(F, F, F, T, F),
    random_effect=c(F, T, F, T, T),
    kernel_scale=c(0, 0, 1e-5, 0, 0),
    name=c("independent", "proxy", "local", "estimated", "proxy, non-adaptive")
  ),
